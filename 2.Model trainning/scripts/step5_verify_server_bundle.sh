#!/usr/bin/env bash
# Step 5 helper: verify the extracted Chinese-Origin 125M server bundle.
set -euo pipefail
...
root = Path(".")
tokenizer_dir = root / "tokenizers" / "chinese_origin_32k_eos"
train_dir = root / "data" / "tokenized" / "chinese_train_full_eos_1024"
valid_dir = root / "data" / "tokenized" / "chinese_valid_full_eos_1024"
config_path = root / "configs" / "step5_chinese_125m_b1024.yaml"
config_4epoch_path = root / "configs" / "step5_chinese_125m_b1024_4epoch.yaml"
...
required_paths = [
    ...
    ("config", config_path),
    ("4epoch config", config_4epoch_path),
...
test_dir = root / "data" / "tokenized" / "chinese_test_full_eos_1024"
if test_dir.exists():
...
print("Server bundle verification passed.")
PY
